...
import { fetchPropertiesListing } from '../../../../../../Backend/Api/api';
...
const PropertyListing = () => {
    ...
    useEffect(() => {
        const fetchProperties = async () => {
            try {
                const propertyData = await fetchPropertiesListing();
    ...
    const handleAction = async (action, property) => {
        ...
        if (newStatus) {
            try {
                ...
                await axios.patch(`http://localhost:5000/properties/${property.propertyID}/status`, { status: newStatus });
    
                
                setProperties((prevProperties) =>
                    prevProperties.map((p) =>
                        p.propertyID === property.propertyID ? { ...p, propertyStatus: newStatus } : p
                    )
                );
                console.log(`${newStatus} property:`, property.propertyID);
            } catch (error) {
                console.error(`Failed to ${newStatus} property:`, error);
            }
        }
    ...
    const propertyDropdownItems = [
        ...
        { label: 'Approve', icon: <FaEdit />, action: 'approve' },
        { label: 'Reject', icon: <FaBan />, action: 'reject' }
    ];
    ...
    return (
        <div>
            ...
            <button className="create-property-button" onClick={() => setIsPropertyFormOpen(true)}>Create New Property</button>
            ...
            {isPropertyFormOpen && (
                <div className="property-form-overlay" onClick={() => setIsPropertyFormOpen(false)}>
                    <div className="property-form-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <PropertyForm onClose={() => setIsPropertyFormOpen(false)} />
                    </div>
                </div>
            )}
        </div>
